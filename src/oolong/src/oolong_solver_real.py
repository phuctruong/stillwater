...
- An optional demo path that can call a local Claude Code HTTP wrapper (see
  `src/cli/src/claude_code_wrapper.py`) and combine it with deterministic CPU counting.
...
def main():
    ...
    if solver.wrapper.server_running:
        ...
        print("   Start the wrapper with: python3 src/cli/src/claude_code_wrapper.py\n")
