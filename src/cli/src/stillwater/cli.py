...
def _repo_root() -> Path:
    # `src/cli/src/stillwater/cli.py` -> repo root is 5 levels up (index 4).
...
def _verify_wish_artifacts(root: Path, wish_id: str) -> tuple[bool, list[str], dict[str, Any]]:
    ...
    candidate_dirs = [
        ...
        root / "data" / "default" / "wishes" / "examples" / "artifacts" / "wishes" / wish_id,
        root / "src" / "cli" / "wishes" / "artifacts" / "wishes" / wish_id,
    ]
...
def _default_twin_skills(root: Path) -> list[str]:
    root_skills = root / "data" / "default" / "skills"
...
def _load_kernel_config(root: Path) -> dict[str, Any]:
    ...
    cfg_path = _resolve_user_path(root, cfg_env) if cfg_env else (root / "src" / "cli" / "kernel_config.yaml")
...
def _kernel_paths(root: Path) -> dict[str, Any]:
    ...
    ext_root = _resolve_user_path(root, ext_env or ext_cfg) if (ext_env or ext_cfg) else (root / "src" / "cli" / "extensions")

    skill_dirs: list[tuple[str, Path]] = [
        ("root", root / "data" / "default" / "skills"),
        ("cli", root / "src" / "cli" / "skills"),
        ("cli_stillwater", root / "src" / "cli" / "skills" / "stillwater"),
    ]
    if ext_root.exists():
        skill_dirs.extend(
            [
    ...
    recipe_dirs = ["src/cli/recipes", "data/default/recipes", "ripples/system", "ripples/project"]
    ...
    if identity_env:
        ...
        identity_dir = root / "src" / "cli" / "identity"
    ...
    papers_dirs: list[str] = ["src/cli/papers", "papers"]
    ...
    default_swarm_settings = root / "src" / "cli" / "settings" / "SWARM-ORCHESTRATION.prime-mermaid.md"
    ...
    return {
        "config_path": str(_resolve_user_path(root, os.environ.get("STILLWATER_KERNEL_CONFIG", ""))) if os.environ.get("STILLWATER_KERNEL_CONFIG", "").strip() else str(root / "src" / "cli" / "kernel_config.yaml"),
...
def _cpu_twin_prepass(*, root: Path, prompt: str) -> tuple[bool, str, dict[str, Any]]:
    ...
    if text == "/wishes" or "list wishes" in lower or "show wishes" in lower:
        wish_files = sorted((root / "src" / "cli" / "wishes").glob("wish.*.md"))
...
def _default_math_universal_config() -> dict[str, Any]:
    return {
        ...
        "heldout_profiles": [
            {
                ...
                "to_year": 2025,
                "required_rung": 65537,
                "oracles_file": "src/cli/tests/math/imo_history_oracles.json",
                "min_pass_ratio": 1.0,
                "require_strict": True,
            }
        ],
        "proof_artifacts": {
            "required": True,
            "cases_file": "src/cli/tests/math/proof_artifact_cases.json",
        ...
        "generalization_profiles": [
            {
                ...
                "to_year": 2025,
                "required_rung": 641,
                "oracles_file": "src/cli/tests/math/imo_history_oracles.empty.json",
                "min_pass_ratio": 0.90,
...
def _run_math_universal_bench_profile(
    *,
    ...
    oracle_raw = profile.get("oracles_file", defaults.get("oracles_file", "src/cli/tests/math/imo_history_oracles.json"))
...
def _imo_phuc_case_scout(case: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "suspect_files_ranked": [
            {"path": "src/cli/src/stillwater/cli.py", "reason": "routing + orchestration execution"},
            {
                "path": "src/cli/settings/SWARM-ORCHESTRATION.prime-mermaid.md",
...
def _imo_memory_next_actions(fail_counts: dict[str, int]) -> list[str]:
    if not fail_counts:
        return [
            "Expand coverage: add new cases in src/cli/tests/math/*.json and rerun benchmark.",
    ...
    if "route" in joined or "action" in joined or "source" in joined:
        actions.append("Review route policy in src/cli/settings/SWARM-ORCHESTRATION.prime-mermaid.md.")
...
def _imo_history_case_scout(*, year: int, problem_id: str, prompt: str) -> dict[str, Any]:
    return {
        ...
        "suspect_files_ranked": [
            {"path": "src/cli/src/stillwater/cli.py", "reason": "history bench runtime + routing"},
            {
                "path": "src/cli/settings/SWARM-ORCHESTRATION.prime-mermaid.md",
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_init_identity.add_argument("--dir", default="src/cli/identity", help="Identity directory.")
    ...
    p_skills_sync.add_argument("--dest", default="src/cli/skills/stillwater", help="Destination directory (repo-relative).")
    ...
    p_wish_list = p_wish_sub.add_parser("list", help="List available wishes in src/cli/wishes and wishes/examples.")
    ...
    p_wish_init.add_argument("--path", default="src/cli/wishes", help="Target directory.")
    ...
    p_recipe_lint.add_argument(
        ...
        action="append",
        default=[],
        help="Directory to lint (repeatable). Defaults to wishes/ripples/recipes/memory/src/cli/wishes.",
    )
    ...
    p_recipe_add.add_argument("--dir", default="src/cli/recipes", help="Target directory.")
    ...
    p_math_universal.add_argument(
        "--config",
        default="src/cli/tests/math/universal_math_gate.json",
    ...
    p_imo_phuc.add_argument(
        "--cases-file",
        default="src/cli/tests/math/imo_qa_cases.json",
    ...
    p_imo_bench.add_argument(
        "--oracles-file",
        default="src/cli/tests/math/imo_history_oracles.json",
    ...
    notebooks = [
        ...
        "notebooks/HOW-TO-CONVENTION-DENSITY-BENCHMARK.ipynb",
        "notebooks/HOW-TO-RIPPLE-LEARNING-BENCHMARK.ipynb",
        "notebooks/HOW-TO-PLUGIN-SDK-BENCHMARK.ipynb",
    ...
    if ns.cmd == "init":
        ...
        if ns.init_cmd == "project":
            ...
            stillwater_skills_dir = root / "data" / "default" / "skills"
    ...
    if ns.cmd == "skills-ab":
        ...
        cfg = SkillsABConfig(
            repo_root=root,
            skills_dir=root / "data" / "default" / "skills",
    ...
    if ns.cmd == "skills":
        ...
        if ns.skills_cmd == "install":
            ...
            if skill_content is None and not ns.no_remote:
                ...
                gh_url = f"https://raw.githubusercontent.com/phuctruong/stillwater/main/data/default/skills/{base_name}"
                try:
            ...
            repo_skills_dir = root / "data" / "default" / "skills"
    ...
    if ns.cmd == "wish":
        if ns.wish_cmd == "list":
            ...
            sources = [
                ("cli_wishes", root / "src" / "cli" / "wishes"),
                ("examples", root / "data" / "default" / "wishes" / "examples"),
            ]
        ...
        if ns.wish_cmd == "init":
            ...
            if ns.level == "l1":
                template_nb = root / "data" / "default" / "wishes" / "templates" / "WISH-NOTEBOOK-TEMPLATE.ipynb"
                template_pm = root / "data" / "default" / "wishes" / "templates" / "WISH-PRIME-MERMAID-TEMPLATE.prime-mermaid.md"
    ...
    if ns.cmd == "recipe":
        ...
        if ns.recipe_cmd == "lint":
            lint_dirs = ns.dir or ["wishes", "ripples", "recipes", "src/cli/recipes", "memory", "src/cli/wishes"]
    ...
    if ns.cmd == "math-universal":
        ...
        defaults: dict[str, Any] = {
            ...
            "oracles_file": str(defaults_raw.get("oracles_file", "src/cli/tests/math/imo_history_oracles.json")).strip(),
        }
        if ns.model:
        ...
        proof_cases_file = _resolve_user_path(root, str(proof_cfg.get("cases_file", "src/cli/tests/math/proof_artifact_cases.json")))
    ...
    if ns.cmd == "run":
        ...
        if skill_name:
            # Try skills/<name>.md, then skills/<name>, then src/cli/skills/<name>.md
            candidates = [
                root / "data" / "default" / "skills" / f"{skill_name}.md",
                root / "data" / "default" / "skills" / skill_name,
                root / "src" / "cli" / "skills" / f"{skill_name}.md",
                root / "src" / "cli" / "skills" / skill_name,
            ]
        ...
        if not response_text:
            try:
                sys.path.insert(0, str(root / "src" / "cli" / "src"))
    ...
    if ns.cmd == "print" or ns.cmd is None:
        ...
        print("  stillwater skills sync --dest src/cli/skills/stillwater --all --force")
        print("  stillwater recipe list")
        print("  stillwater recipe add twin_orchestration --dir src/cli/recipes")
        ...
        print("  stillwater init identity-stack --dir src/cli/identity --force")
