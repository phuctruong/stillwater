"""
src/store/rung_validator.py — Evidence bundle validator for Stillwater Store submissions.
...
class RungValidator:
    ...
    def verify_rung(self, rung_claimed: Optional[int], bundle_path: Path) -> str:
        ...
        # Rung 65537: above + security_scan.json with status=PASS|GREEN
        if not (bundle_path / "security_scan.json").exists():
            return _INVALID

        try:
            scan = json.loads((bundle_path / "security_scan.json").read_text(encoding="utf-8"))
            if not isinstance(scan, dict) or scan.get("status") not in {"PASS", "GREEN"}:
                return _INVALID
        except (json.JSONDecodeError, OSError):
            return _INVALID
