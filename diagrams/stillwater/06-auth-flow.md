...
Stillwater Store. Based entirely on `store/auth.py`.
...
- `store/auth.py` — `generate_api_key()`, `_hash_key()`, `validate_key_format()`, `lookup_api_key()`, `require_api_key()`, `check_rate_limit()`, `apply_reputation()` (lines 1-188)
- `store/db.py` — `get_api_key_by_hash()`, `count_recent_submissions()`, `update_api_key()`
