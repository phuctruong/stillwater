...
flowchart TD
    ...
    CLI_SKILLS["cli/skills/"]
    CLI_SW_SKILLS["cli/skills/stillwater/"]
...
- `cli/src/stillwater/cli.py` — `main()` function, all `sub.add_parser()` calls (lines 5268–5822)
- `cli/src/stillwater/llm_client.py` — `LLMClient`, `_log_call()`, `get_call_history()`
- `cli/src/stillwater/provider_registry.py` — `PROVIDERS` dict, `_MODEL_PROVIDER_MAP`, `_OLLAMA_PREFIXES`
- `cli/src/stillwater/usage_tracker.py` — `SessionUsageTracker`, `SW5_ITERATION_REDUCTION_PCT=40`
- `cli/src/stillwater/session_manager.py` — `Session` dataclass, TTL=86400s
- `cli/src/stillwater/llm_cli_support.py` — Ollama probing helpers
