...
flowchart TD
    ...
    subgraph CLI ["stillwater CLI  (cli/src/stillwater/)"]
    ...
    subgraph STORE ["Stillwater Store  (store/)"]
...
- `cli/src/stillwater/cli.py` — main CLI entry point, command dispatch
- `cli/src/stillwater/llm_client.py` — universal LLM client, call logging
- `cli/src/stillwater/session_manager.py` — session lifecycle
- `cli/src/stillwater/usage_tracker.py` — cost tracking
- `cli/src/stillwater/skills_ab.py` — A/B benchmark harness
- `cli/src/stillwater/provider_registry.py` — provider routing table
- `cli/src/stillwater/providers/` — individual provider implementations
- `admin/server.py` — HTTP admin server with security guards
- `admin/llm_portal.py` — LLM portal, OpenAI-compat proxy
- `store/models.py` — Pydantic data models
- `store/db.py` — in-memory store with JSON persistence
- `store/auth.py` — API key generation, HMAC validation, rate limiting
- `store/rung_validator.py` — evidence bundle validation
