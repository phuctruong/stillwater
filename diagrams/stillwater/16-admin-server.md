...
flowchart TD
    ...
    GET_CMD --> SETENV["Set PYTHONPATH=cli/src\nin subprocess env"]
...
flowchart LR
    ...
    CATALOG --> G6["recipes\n→ cli/recipes/*.md\n   cli/extensions/recipes/*.md"]
    CATALOG --> G7["skills\n→ cli/extensions/skills/*.md"]
    CATALOG --> G8["personas\n→ cli/extensions/personas/*.md"]
    CATALOG --> G9["identity\n→ cli/identity/*.md\n   cli/extensions/identity/*.md"]
    CATALOG --> G10["settings\n→ cli/settings/*.md"]

    EXTRA["EXTRA_EDITABLE_FILES\n(not in catalog groups)"]
    EXTRA --> E1["llm_config.yaml"]
    EXTRA --> E2["cli/extensions/splash.txt"]
    EXTRA --> E3["CLAUDE.md"]
...
sequenceDiagram
    ...
    SERVER->>SERVER: Count cli/recipes/*.md + skills/*.md
...
- Subprocess execution: cwd=REPO_ROOT, PYTHONPATH=cli/src, timeout=30s
