...
- "Correctly" = the word matches its definition in magic-words/ AND is used in context.
...
- Submit the convention card from Stage 2 (or a new one) to `magic-words/`.
...
L: Convention in magic-words/ = +15
...
- magic-words/ PR or commit showing convention added
...
- ≥3 convention cards in `magic-words/` with active adoption
- `glow_score.json` ≥ 85
...
- ≥5 conventions in magic-words/ with adoption evidence
- `glow_score.json` ≥ 95
...
3. **Convention death:** Convention removed from magic-words/ due to conflict with prime-safety → LEC score does not count that convention. Must be replaced.
...
- `magic-words/` — the LEC registry (where convention cards live)
