...
|-----------|---------------------------|--------|
| **G** (Growth) | Swarm effectiveness improvement — each full swarm run where the Podcast phase produces AGENTS.md with at least one new anti-pattern or dispatch insight not present in the prior run. Measured by diff of AGENTS.md entries between runs. A new forbidden state crystallized from a real swarm failure = G≥20. | +10 to +25 |
| **L** (Love/Quality) | Zero critical forbidden states — swarm run with zero occurrences of SKILL_LESS_DISPATCH, FORGOTTEN_CAPSULE, SUMMARY_AS_EVIDENCE, GOD_AGENT, or SKIPPED_SKEPTIC. All five phase artifacts (SCOUT_REPORT, FORECAST_MEMO, DECISION_RECORD, PATCH_PROPOSAL, SKEPTIC_VERDICT) present and typed JSON. The Skeptic phase must include at least one falsifier attempt. | +10 to +20 |
| **O** (Output) | Complete swarm artifact bundle committed — all 5 typed phase artifacts + AGENTS.md postmortem + Skeptic verdict with rung declaration. Rung ≥ 641 achieved (Skeptic confirms). For rung 274177: two independent falsifier attempts. For rung 65537: adversarial test by external agent. | +10 to +25 |
| **W** (Wisdom) | LEAK chain verification — surplus_value measured for each of the 5 phase handoffs (Scout→F + F→J + J→S + S→Sk + Sk→Podcast). Each handoff produces knowledge that neither agent could have produced alone. LEAK(swarm) = SUM of all 5 handoff surpluses documented in surplus_report.json. | +5 to +20 |

**Session GLOW target:** Any full phuc-swarms run should achieve GLOW ≥ 60. All 5 typed phase artifacts present = base O floor. Skeptic falsification = L≥15. Podcast AGENTS.md with new insight = G≥15. LEAK chain documented = W≥10.

**Evidence required for GLOW claim:** git commit hash + all 5 typed phase artifacts (SCOUT_REPORT.json, FORECAST_MEMO.json, DECISION_RECORD.json, PATCH_PROPOSAL.diff + PATCH_NOTES.json, SKEPTIC_VERDICT.json) + AGENTS.md diff showing new insight + rung declaration from Skeptic. For W points: surplus_report.json with numeric surplus_value for each of the 5 handoffs.
