...
./cli/stillwater-cli.sh imo-history bench --from-year 2020 --to-year 2024 --fetch-missing --model llama3.1:8b --required-rung 65537
./cli/stillwater-cli.sh imo-history bench --from-year 2020 --to-year 2024 --fetch-missing --model llama3.1:8b --required-rung 65537 --oracles-file cli/tests/math/imo_history_oracles.json
./cli/stillwater-cli.sh imo-history autolearn --from-year 2020 --to-year 2024 --fetch-missing --model llama3.1:8b --required-rung 65537 --max-iterations 3 --oracles-file cli/tests/math/imo_history_oracles.json
./cli/stillwater-cli.sh math-universal --config cli/tests/math/universal_math_gate.json --json
./cli/stillwater-cli.sh qa-math-universal
```
...
Config source:
- `cli/tests/math/imo_qa_cases.json` (question prompts + match needles)
- `cli/tests/math/imo_history_defaults.json` (history sweep defaults)
- `cli/tests/math/imo_history_oracles.json` (historical proof-grade oracle targets)
- `cli/tests/math/imo_history_oracles.empty.json` (empty oracle baseline for no-hint generalization)
- `cli/tests/math/proof_artifact_cases.json` (deterministic proof-artifact checks)
- `cli/tests/math/universal_math_gate.json` (universal-claim gate config)
...
2. `tool_assisted` lane scores `N/N`, where `N` is the number of configured cases in `cli/tests/math/imo_qa_cases.json`
...
```bash
./cli/stillwater-cli.sh twin "In any 2-coloring of K6, what exists by R(3,3)=6?" --json
```

Pure model route (expected `source=LLM`, `action=ollama_chat`):
```bash
./cli/stillwater-cli.sh twin "In any 2-coloring of K6, what exists by R(3,3)=6?" --llm-only --model llama3.1:8b --json
```

Kernel/swarm settings disclosure:
```bash
./cli/stillwater-cli.sh twin "/kernel" --json
```
