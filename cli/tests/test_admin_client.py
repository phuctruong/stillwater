...
Run with:
    pytest cli/tests/test_admin_client.py -v
...
class TestGetFile:
    ...
    def test_read_real_file_returns_dict(self, admin_client):
        result = admin_client.get_file("skills/prime-safety.md")
        assert isinstance(result, dict)
        assert result.get("ok") is True
    ...
    def test_path_echoed_in_response(self, admin_client):
        result = admin_client.get_file("skills/prime-safety.md")
        assert result["path"] == "skills/prime-safety.md"
    ...
    def test_nonexistent_file_raises_error(self, admin_client):
        with pytest.raises(AdminClientError) as exc_info:
            admin_client.get_file("skills/does_not_exist_xyzzy.md")
...
class TestSaveFile:
    ...
    def test_round_trip_write_and_read(self, admin_client):
        ...
        test_rel = "skills/zzz_client_test_save_temp.md"
    ...
    def test_save_returns_sha256(self, admin_client):
        ...
        test_rel = "skills/zzz_client_sha256_temp.md"
    ...
    def test_save_returns_correct_size(self, admin_client):
        ...
        test_rel = "skills/zzz_client_size_temp.md"
        content = "# size test\n"
...
class TestCommunityAPI:
    ...
    def test_community_link_valid_email(self, admin_client):
        result = admin_client.community_link("clienttest@example.com")
        assert result.get("ok") is True

    def test_community_link_returns_link_object(self, admin_client):
        result = admin_client.community_link("linkobject@example.com")
        assert "link" in result
        link = result["link"]
        assert link.get("email") == "linkobject@example.com"
        assert link.get("status") == "magic_link_sent_mock"

    def test_community_link_has_api_key(self, admin_client):
        result = admin_client.community_link("apikey@example.com")
        link = result["link"]
        assert "api_key" in link
        assert len(link["api_key"]) > 0

    def test_community_link_has_login_link_stub(self, admin_client):
        result = admin_client.community_link("loginstub@example.com")
        link = result["link"]
