...
Kernel responsibilities (`cli/src/stillwater/`):
...
Extension responsibilities (default `cli/extensions/` or `STILLWATER_EXTENSION_ROOT`):
- skills (`skills/*.md`)
...
4. Use `cli/kernel_config.yaml` for file-path defaults; use env vars for per-run overrides.
...
- `cli/settings/SWARM-ORCHESTRATION.prime-mermaid.md` (externalized swarm contract)
...
- `cli/tests/math/imo_qa_cases.json` (editable case config)
- `cli/tests/math/imo_history_defaults.json` (editable history defaults)
- `cli/tests/math/imo_history_oracles.json` (editable historical oracle targets)
- `cli/tests/math/proof_artifact_cases.json` (deterministic proof-artifact config)
...
Identity files live in `cli/identity/` and can be overridden in extensions:
...
1. keep kernel in `~/projects/stillwater/cli/src/stillwater/`
