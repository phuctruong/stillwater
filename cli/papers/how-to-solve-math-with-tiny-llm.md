## Math Secret Sauce (Software 5.0): How to Build a World-Class Math System on Open-Source 8B Models
...
1. **Proof equivalence verifier** (semantic grading)
   baseline implemented: normalized matcher + per-case `aliases`/`concepts`/`required_sections` in `cli/tests/math/imo_qa_cases.json`
