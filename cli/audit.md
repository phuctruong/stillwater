...
Date: 2026-02-19  
Auditor: Codex (GPT-5)  
Scope: `cli/` runtime, tests, notebooks, wishes/recipes/docs integration
...
3. Council settings are externalized in `cli/settings/SWARM-ORCHESTRATION.prime-mermaid.md` under `expert_council.*`.
...
5. Historical oracle targets are now externalized via `cli/tests/math/imo_history_oracles.json` and `imo-history bench --oracles-file ...`.
...
Latest verification:
- `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q cli/tests` -> `60 passed, 4 skipped`.
- `./cli/stillwater-cli.sh qa-imo` -> tool-assisted `6/6`, strict pass true.
- `./cli/stillwater-cli.sh imo-history bench --from-year 2022 --to-year 2022 --max-problems 2 --required-rung 65537 --model llama3.1:8b --json` -> strict fail (expected), `rung_achieved=274177`, `oracle_configured_cases=0`.
- `./cli/stillwater-cli.sh imo-history bench --from-year 2022 --to-year 2022 --max-problems 2 --required-rung 641 --model llama3.1:8b --json` -> strict pass true.
- `./cli/stillwater-cli.sh imo-history oracle-status --from-year 2022 --to-year 2024 --oracles-file cli/tests/math/imo_history_oracles.json --json` -> `oracle_ready=2/18` (11.11%).
...
10. Swarm orchestration settings are externalized in `cli/settings/SWARM-ORCHESTRATION.prime-mermaid.md` and runtime injections are auditable via `system.prompt.md` + `injection-manifest.json`.
...
- Symptom: `./cli/stillwater-cli.sh twin '/recipes' --json` returned non-zero when configured recipe dirs included absolute paths outside repo.
- Root cause: `relative_to(root)` was used without `ValueError` fallback in CPU prepass recipe listing.
- Fix: added safe fallback for absolute paths in `cli/src/stillwater/cli.py`.
- Regression test added: external `recipe_dirs` + twin `/recipes` path in `cli/tests/test_cli_kernel_config.py`.
- Verification: command now returns `rc=0` and includes external recipes in response.

2. **High (Fixed): root notebook import path broke after src->cli/src refactor**
- Symptom: `PHUC-SKILLS-SECRET-SAUCE.ipynb` failed with `ModuleNotFoundError: stillwater` when executed from root.
- Root cause: notebook hardcoded `sys.path` to `./src`, but runtime package now lives in `./cli/src`.
- Fix: updated notebook to detect both layouts (`src/` and `cli/src/`) before import.
...
- Regression test added: `test_twin_cpu_imo_fact_router` in `cli/tests/test_cli_twin_features.py`.
...
- `cli/kernel_config.yaml` intentionally references local external dirs (solace projects).
...
- `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 pytest -q cli/tests` -> **31 passed, 4 skipped**

## 2) Runtime QA scripts

- `./cli/stillwater-cli.sh qa-fast` -> PASS
- `./cli/stillwater-cli.sh qa-notebooks` -> PASS
- `./cli/stillwater-cli.sh qa-secret-sauce` -> PASS
