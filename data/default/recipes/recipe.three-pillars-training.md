...
4. Name it. Write a magic word definition following the data/default/magic-words/ format:
   ```
   WORD: <the name>
   TIER: 0|1|2|3
...
3. Submit the convention card to `data/default/magic-words/` (or the team convention registry).
...
- PR or diff showing convention_card committed to data/default/magic-words/
- `triangle_check.json` — triangle law verification for the convention

**Pass condition:** Convention in data/default/magic-words/ AND 3 new usages AND triangle_check passes.
...
| 7 | data/default/magic-words/ PR, triangle_check.json | 3 new usages + triangle law pass |
...
| **O** (Output) | PATCH_DIFFs committed, surplus_proof.json with numeric values, data/default/magic-words/ PR merged | +25 for committed artifacts |
