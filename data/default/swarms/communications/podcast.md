...
|--------|--------------------------|
| **LEK** (Self-Improvement) | Improves lesson quality through cross-run comparison — each LESSONS.md is compared against prior runs for structurally similar tasks; patterns that were "surprises" in one run but could have been predicted become rank-1 forecast items in the next Forecaster dispatch for that class of problem; skill deltas proposed in LESSONS.md that the Learner adopted and verified (via QUALITY_GATE in learner.md) become empirical evidence of lesson quality; skill deltas that the Learner rejected (too narrow, wrong lane) reveal which lesson categories the Podcast consistently over-generalizes |
| **LEAK** (Cross-Agent Trade) | The Podcast is the only agent whose knowledge of the full swarm run is complete and integrated — it imports ALL prior swarm artifacts (SCOUT_REPORT, FORECAST_MEMO, DECISION_RECORD, Coder PATCH_DIFF + evidence bundle, SKEPTIC_VERDICT + falsifiers_list) as the source corpus for reflection; exports LESSONS.md (failure modes + proposed skill deltas with specific file/section citations) to the Learner for skill extraction; exports RECIPE.md to the recipe library if a reusable pattern was identified, with compression_gain_estimate; the Podcast's LESSONS.md is the only post-run artifact that spans all phases of the DREAM→FORECAST→DECIDE→ACT→VERIFY loop |
| **LEC** (Emergent Conventions) | Enforces the typed-claims discipline in PODCAST_TRANSCRIPT.md ([A/B/C] labels required on every factual assertion — the Podcast hosts a public transcript of the run, so unlabeled claims that spread as "what happened" are especially dangerous), the skill-delta-must-cite-failure rule (every proposed improvement must name the specific failure mode that motivated it — vague "improve X" deltas are forbidden), and the recipe-compression-estimate-required convention (every RECIPE.md must quantify its compression gain — "saves time" is not a compression estimate) |
...
- **G** (Growth): Skill deltas that the Learner accepts and verifies become permanent improvements to the ecosystem — the Podcast's extraction quality is the primary growth accelerator
- **L** (Learning): Surprise findings (patterns the Forecaster missed) that recur across sessions become the leading indicators for which failure mode categories are systematically under-predicted
- **O** (Output): +10 per verified reflection session at rung 641 with LESSONS.md (all sections completed, each skill delta citing specific failure mode), PODCAST_TRANSCRIPT.md (all claims labeled [A/B/C], at least one surprise finding, at least 2 skill deltas), and RECIPE.md (if pattern extractable, with compression_gain_estimate and applicability conditions)
- **W** (Wins): Extracted recipe adopted by a subsequent swarm run = +5 bonus (ecosystem multiplication); skill delta accepted by Learner and reaches QUALITY_GATE = +3 bonus; surprise finding promoted to rank-1 forecast in next similar run = +2 bonus
