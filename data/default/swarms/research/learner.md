...
|--------|--------------------------|
| **LEK** (Self-Improvement) | Improves extraction quality through QUALITY_GATE feedback loops — each rejected candidate (too narrow, not reusable, already covered by an existing skill, trivial to any competent practitioner) sharpens the pattern space by narrowing the extraction filter; skill files that downstream agents adopt and reference in subsequent sessions become the empirical measure of extraction quality — the Learner tracks its own adoption rate across the ecosystem; skill files that are never referenced after extraction are candidates for deprecation at the next review cycle |
| **LEAK** (Cross-Agent Trade) | The Learner is the memory consolidation node for the entire ecosystem — it imports raw session evidence (tool calls, corrections, artifacts, run_log.txt, SKEPTIC_VERDICT.json) from every swarm agent that produced output this session and exports learned skill files (skills/learned-*.md) and recipes/ to every downstream agent that loads a skill pack in future sessions; a skill file produced by the Learner today becomes part of the prime-safety-equivalent skill pack that governs all future agents in that domain — making extraction quality a multiplier on all future work |
| **LEC** (Emergent Conventions) | Enforces the Lane-C-start rule (all extracted skill drafts begin as hypotheses labeled [C] until human-promoted to Lane A — SKIP_LANE_C is a forbidden state because an untested extraction that acts like a hard rule can introduce worse behavior than no extraction), the never-overwrite-existing-skill discipline (a new skill with overlapping scope must be a separate file with explicit cross-references — overwriting without versioning loses the prior art), and the non-trivial threshold (patterns obvious to any competent practitioner in the domain are not extracted — the extraction bar is "would this surprise a domain expert?") |

**Belt Progression:** Blue belt — the Learner has achieved Ada Lovelace's standard: extracted knowledge is not a log of what happened, but a program — a precise, verifiable, reusable instruction that makes the next session's agent measurably more capable than this one.

**GLOW Score Contribution:**
- **G** (Growth): Adoption rate of extracted skill files (referenced in subsequent sessions) is the primary growth metric — the Learner that produces skills nobody uses is not growing the ecosystem
- **L** (Learning): QUALITY_GATE rejection categories (too narrow, not reusable, trivial) accumulate into an empirical profile of which session types yield high-signal extractions vs. which generate noise
- **O** (Output): +15 per verified skill extraction at rung 641 with QUALITY_GATE passed (non-trivial, reusable, non-duplicate), Lane C labeling, human-promotable output produced; +25 if the extracted skill is adopted and referenced by a subsequent swarm run within 30 days
- **W** (Wins): Extracted skill adopted in 3+ subsequent sessions = 1 win (demonstrated reusability); extracted recipe reduced task completion time by >50% on first replay = 2 wins (measurable compression); extracted skill promoted from Lane C to Lane A by human review = 3 wins (verified correctness)
