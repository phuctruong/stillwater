...
elif task contains ["schema", "database", "SQL"]:
    load persona: codd
...
|--------|--------------------------|
| **LEK** (Self-Improvement) | Improves persona-task matching through GLOW_SCORE feedback loops — each glow_score.json records which persona was loaded for which task domain; low G scores (minimal growth) combined with high L scores (heavy learning) reveal persona-domain mismatches where the agent learned more than it built; accumulated glow_score.json histories across sessions expose which persona-domain pairings consistently yield the highest G+O scores (maximum output quality) vs. which pairings introduce style noise without domain expertise uplift |
| **LEAK** (Cross-Agent Trade) | Exports glow_score.json (all four G/L/O/W dimensions justified with persona and northstar alignment) to the DISPATCH_LOG and ecosystem belt tracker as the evidence that persona loading produced measurable value; exports PATCH_DIFF + evidence bundle (identical to base Coder: tests.json + repro_red.log + repro_green.log + plan.json) to the Skeptic for adversarial verification; the GLOW breakdown in commit messages propagates persona alignment and northstar metric advancement to the git log as machine-readable evidence trail for the Final Audit |
| **LEC** (Emergent Conventions) | Enforces the persona-must-be-loaded rule (PERSONA_THEATER is forbidden — persona voice must include domain-specific guidance relevant to this specific task, not just stylistic quotes), the GLOW-conservative-scoring convention (G=25 requires a major module at rung 274177+; a 5-line helper function is G=5 maximum — inflation destroys the signal value of GLOW scores), and the persona-is-style-not-authority doctrine (persona voice and domain expertise never override prime-safety gates or DECISION_RECORD scope — PERSONA_SCOPE_CREEP is forbidden) |
...
- **G** (Growth): GLOW G scores across sessions for each persona-domain pairing build an empirical table of which combinations deliver the most ecosystem growth per task
- **L** (Learning): Sessions with low G but high L scores reveal persona-task mismatches where the agent spent more time learning than building — these inform better dispatch decisions
- **O** (Output): +15 per verified implementation at rung 641 with glow_score.json (all four dimensions justified), persona_loaded != null, northstar_metric_advanced explicitly stated, and evidence bundle present; +25 at rung 274177 with domain match verified and persona knowledge specifically cited in the evidence narrative
- **W** (Wins): Persona domain knowledge directly prevented a bug the base Coder would have introduced = 1 win; glow_score.json G dimension scored 20+ (major module delivered) = 2 wins; commit GLOW breakdown adopted as standard format in the Final Audit = +1 ecosystem integration bonus
