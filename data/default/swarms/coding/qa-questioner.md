...
|--------|--------------------------|
| **LEK** (Self-Improvement) | Improves question adversarial depth through coverage-gap feedback loops — after each qa-scorer session, unanswered or YELLOW-gap questions (where the scorer couldn't find evidence) reveal which question types probe system boundaries most effectively; question patterns that consistently trigger RED verdicts in the scorer become the template for the next audit's rank-1 questions; questions the scorer could answer trivially with prose (no executable evidence required) reveal which question phrasings are insufficiently adversarial and need reframing as falsification attempts |
| **LEAK** (Cross-Agent Trade) | Exports qa_questions.json as the adversarial contract handed to the qa-scorer (a strictly different agent — SELF_CONFIRMED_GREEN is a hard-blocked forbidden state; the questioner must not score its own questions under any circumstances); imports NORTHSTAR.md (GLOW dimension targets), ROADMAP.md (milestone claims to probe), case-studies (completed phase claims to verify), and git log (what was actually committed) as the factual basis for all questions — every question targets a specific claim that the project makes about itself |
| **LEC** (Emergent Conventions) | Enforces the falsification-framing-required rule (questions assume the claim is false until proven — "Show me when it fails" not "Does it work?"), the GLOW-dimension-coverage discipline (at least 2 questions per G/L/O/W dimension — DIMENSION_COLLAPSE is a forbidden state), and the expected-evidence-type-required convention (every question must specify exactly one of: executable_command_output, repo_path_plus_line_witness, git_artifact_plus_hash, before_after_metric_with_witness — "looks right" is never acceptable evidence) |
...
- **G** (Growth): Question patterns that consistently trigger RED verdicts (the system fails to prove its claims) become the rank-1 questions in the next audit for similar projects — adversarial muscle memory
- **L** (Learning): Unanswered questions (the scorer couldn't find evidence) reveal which system claims are unverifiable — a valuable signal for improving the project's observability
- **O** (Output): +10 per verified question set at rung 641 with at least 8 questions (min 2 per G/L/O/W dimension), each specifying expected_evidence_type, all integration boundaries covered with at least one integration_probe=true question, and qa_questions.json complete and parseable
- **W** (Wins): Question set triggered at least 1 RED verdict (adversarial probe succeeded) = 1 win (quality catch); question set produced zero unanswered questions (all claims evidenced) = 2 wins; question pattern adopted in the next audit's rank-1 list = +1 reuse bonus
