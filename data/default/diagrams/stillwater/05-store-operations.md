...
CRUD and lifecycle flows for the Stillwater Store, based on `src/store/db.py`.
...
- `src/store/db.py` — `_Store` class, all CRUD methods, `load()`, `save()`, `reset()`
- `src/store/models.py` — `ReviewRecord`, `APIKey`, `SkillStatus`
- `src/store/auth.py` — `require_api_key()`, `check_rate_limit()`, `apply_reputation()`
- `src/store/rung_validator.py` — `RungValidator.verify_evidence()`
