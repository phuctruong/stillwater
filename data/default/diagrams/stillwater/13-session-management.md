...
- `~/projects/stillwater/src/cli/src/stillwater/session_manager.py` — full `SessionManager` class: `Session` dataclass, `create_session()`, `get_session()`, `close_session()`, `list_active()`, `list_all()`, `purge_expired()`, `session_count()`
- `~/projects/stillwater/admin/session_manager.py` — admin variant
- `~/projects/stillwater/src/store/rung_validator.py` — `RungValidator.verify_evidence()` used with session's evidence_dir
