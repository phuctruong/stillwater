...
flowchart LR
    ...
    subgraph OSS ["Public / Open Source"]
        ...
        PZIP["pzip\nUniversal compression\nStorage optimization\nEconomics documented in case studies"]
    end

    subgraph PRIVATE ["Private"]
        SCLI["solace-cli\nExtends stillwater/cli\nOAuth3 vault\nAES-256-GCM tokens\nTwin orchestration\nCloud backend"]
        PVIDEO["pvideo\nIF Theory engine\nPhysics video/avatar\nSpecialized private modules"]
...
- `src/store/auth.py` — `sw_sk_` key format, HMAC validation
- `admin/llm_portal.py` — LLM provider routing documentation
- `src/cli/src/stillwater/provider_registry.py` — Together.ai, OpenRouter, Anthropic, OpenAI providers
