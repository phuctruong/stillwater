...
**Description:** The Stillwater LLM Portal (`admin/llm_portal.py`) is a FastAPI-based web application running on `localhost:8788`. It exposes an OpenAI-compatible proxy interface for all configured LLM providers and includes a dark-theme web UI for interactive testing. The universal LLM client (`src/cli/src/stillwater/llm_client.py`) abstracts all provider differences behind a single API.
...
- `src/cli/src/stillwater/llm_client.py` — Universal LLMClient, llm_call, llm_chat, _log_call
- `src/cli/src/stillwater/provider_registry.py` — High-level PROVIDERS dict, routing table
- `src/cli/src/stillwater/providers/__init__.py` — Provider class registry, PROVIDER_PRIORITY, list_available_providers
- `src/cli/src/stillwater/providers/anthropic_provider.py` — Anthropic backend
- `src/cli/src/stillwater/providers/openai_provider.py` — OpenAI backend
- `src/cli/src/stillwater/providers/together_provider.py` — Together.ai backend
- `src/cli/src/stillwater/providers/openrouter_provider.py` — OpenRouter backend
- `src/cli/src/stillwater/providers/ollama_provider.py` — Local Ollama backend
