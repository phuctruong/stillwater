...
## Phase 1 Wiring Protocol

### CPU-First Response Selection

When Phase 1 classifies a non-task label with confidence >= threshold (0.70):

1. **Label Match**: Look up label in `data/default/smalltalk/responses.jsonl`
2. **Level Gate**: Select responses at or below current Van Edwards level
   - New session, no history → Level 1 only (Surface/Safe)
   - Returning user, some history → Level 1-2 (Surface + Dopamine)
   - Long relationship, high GLOW → Level 1-3 (all levels)
3. **Warmth Calibration**: Select warmth score matching context
   - Error state → warmth 1-2 (competence over warmth)
   - Normal state → warmth 2-3 (balanced)
   - Achievement state → warmth 3-4 (celebrate)
   - Never exceed warmth 4 without Level 3 relationship
4. **Dedup**: Skip responses used this session
5. **Return**: Emit response immediately (<50ms, all local)

### Low Confidence Fallback (confidence < threshold OR unknown)

When Phase 1 cannot classify with confidence:

1. **Gift Selection**: Randomly select from `jokes.json` or `facts.json`
   - Alternate: joke → fact → joke → fact (don't serve same type twice in a row)
   - Tag match: prefer jokes/facts matching current project context
2. **Append redirect**: Add "What can I help with?" or similar redirect
3. **Return**: Emit gift + redirect immediately

### LLM Background Enrichment

In parallel with CPU response:

1. **LLM classifies**: Send input to Haiku for proper classification
2. **LLM generates**: If label is non-task, LLM generates a better response
3. **Save for next turn**: Write to `data/custom/smalltalk/llm-enrichment.jsonl`
   - Format: same as responses.jsonl but with `"added_by": "llm-enrichment"`
4. **Next session**: On startup, merge llm-enrichment.jsonl into response pool
5. **Never serve LLM response on current turn** — this prevents latency
...
- Never after failure (that's patronizing)
- Warmth 3-4 for normal completions
- Warmth 5 only after demonstrably difficult tasks
- Skip compliment if last response was already warm (>= warmth 4)
...
Rules:
- Show one reminder at session start (if past session data exists)
- Reference specific past work (not generic)
- Templates in `data/default/smalltalk/reminders.jsonl`
- Requires: session history in `data/custom/session-history.jsonl`

### Data Registry Convention

```
data/default/             ← shipped defaults (pip install, git clone)
  smalltalk/
    responses.jsonl       ← 150+ response templates by label + level + warmth
    compliments.jsonl     ← 30 calibrated compliments (plant watering rule)
    reminders.jsonl       ← 15 past-session callback templates
    config.jsonl          ← system configuration (thresholds, limits)
  jokes.json              ← 15 programming jokes (gift fallback)
  facts.json              ← 20 stillwater facts (gift fallback)

data/custom/              ← user overrides + LLM enrichment
  smalltalk/
...
- `data/default` = shipped with the system, version-controlled, read-only at runtime
- `data/custom` = user overrides and LLM-generated enrichment, writable at runtime
- Custom files override default files when both exist (custom wins)
