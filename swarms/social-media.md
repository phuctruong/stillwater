...
|--------|--------------------------|
| **LEK** (Self-Improvement) | Improves hook quality through retention-analysis prediction-vs-actual feedback loops — each RETENTION_ANALYSIS.md predicts drop points before production with a specific percentage drop; after content is published, actual retention data is compared against predicted drop points to calibrate the phuc-forecast failure mode taxonomy for the next content brief; title variants that consistently underperform across platforms become negative training signal for future brainstorming, narrowing which hook patterns actually deliver on their promise vs. which merely generate clicks that churn |
| **LEAK** (Cross-Agent Trade) | Exports CONTENT_BRIEF.md + HOOK.md as the production-ready content contract handed to human creators or downstream Writer agents (the HOOK.md is the single most portable artifact — any agent producing content for this project should start here); exports RETENTION_ANALYSIS.md to the Writer as the structural guide for long-form content (predicted drop points at intervals become the placement guides for re-hooks and section transitions); the phuc-forecast failure modes in RETENTION_ANALYSIS.md become premortem inputs for the next social media campaign planning session |
| **LEC** (Emergent Conventions) | Enforces the hook-must-deliver-on-promise rule (BAIT_AND_SWITCH is a forbidden state — if the hook promises X, the content must deliver X; writing the hook last after content is outlined prevents this mismatch), the three-title-variants-minimum discipline (one title is not a brief — A/B testing requires options with different psychological approaches), and the retention-analysis-is-not-optional convention (every content brief includes a predicted retention curve with at least 3 drop point mitigations — RETENTION_BLIND is a forbidden state) |
...
- **G** (Growth): Retention prediction calibration data (predicted vs. actual drop points) accumulates into an empirical model of which content structures hold attention on each platform
- **L** (Learning): Title variants that consistently underperform across multiple A/B tests reveal which psychological hook patterns fail to deliver, narrowing the creative search space
- **O** (Output): +8 per verified content brief at rung 641 with CONTENT_BRIEF.md (all sections), HOOK.md (verbatim hook with 3-question validation), THUMBNAIL_CONCEPT.md (one-frame story, readable at 100px), RETENTION_ANALYSIS.md (min 3 drop points with percentage predictions and mitigations), and at least 3 title variants with no MISLEADING_HOOK or BAIT_AND_SWITCH forbidden states
- **W** (Wins): Content achieved retention above predicted baseline = 1 win (model was pessimistic); all predicted drop points were within 10% of actual = 2 wins (model was calibrated); hook/title produced highest click-through of any variant tested = +1 win
