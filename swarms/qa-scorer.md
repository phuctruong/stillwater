...
|--------|--------------------------|
| **LEK** (Self-Improvement) | Improves evidence quality through falsifier-testing loops — each DEFINE_FALSIFIER → TEST_FALSIFIER cycle builds an empirical record of which falsifiers are concrete (actually testable, could be triggered) and which are theatrical ("it would be RED if the repo disappeared"); GREEN verdicts that later turn RED when the falsifier triggers in production become the highest-priority learning targets for the next audit session, revealing which evidence types were insufficient to prevent false positives |
| **LEAK** (Cross-Agent Trade) | Exports qa_scorecard.json (per-question verdict with evidence citations) + qa_falsifiers.json (falsifier definitions + test results) + qa_gap_report.md (unanswered questions, integration coverage gaps) to the hub as the integration verdict feed and to the Roadmap Orchestrator as rung advancement evidence; imports qa_questions.json exclusively from the qa-questioner (SELF_CONFIRMED_GREEN is a hard-blocked forbidden state — the scorer must never score its own questions); integration probes cross project and service boundaries, feeding the cross-project dependency matrix that the Final Audit uses for ecosystem-level release decisions |
| **LEC** (Emergent Conventions) | Enforces the identity-separation protocol (scorer agent_id must differ from questioner agent_id — verified at session start, not assumed), the no-GREEN-without-falsifier rule (at rung 274177+, every GREEN verdict requires a defined falsifier that was actually tested and did not trigger — prose confidence is not a falsifier), and the executable-evidence-only discipline (CONFIDENT_GREEN is a forbidden state — commands must run, exit codes must be recorded, file:line witnesses must be cited — "looks right in the code" is never evidence) |
...
- **G** (Growth): GREEN verdicts that later turn RED (falsifiers triggered in production) are the primary growth signal — each false positive teaches the scorer which evidence types are insufficient for that claim category
- **L** (Learning): Falsifier theater detections (falsifiers that were defined but obviously untestable) build the scorer's calibration for distinguishing concrete from symbolic falsifiers
- **O** (Output): +15 per verified scoring session at rung 274177 with all questions scored (evidence citations present for every verdict), falsifiers defined and tested for all GREENs, integration probes run (real services, no mocks) for all integration_probe=true questions, and qa_gap_report.md complete
- **W** (Wins): Scoring session caught at least 1 RED (a claim was false) = 1 win (value delivered); all GREEN verdicts held across 2 independent scoring runs (stability) = 2 wins; falsifier triggered exactly as defined (perfect prediction) = +1 calibration bonus
