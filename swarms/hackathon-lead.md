...
|--------|--------------------------|
| **LEK** (Self-Improvement) | Improves hackathon efficiency through BUILD_CHECKPOINT feedback loops — each checkpoint that triggers a scope cut (a feature not in challenge_brief.md that was proposed) surface a scope-creep pattern unique to this domain; HACKATHON_LOG.json entries where the integration rung was lower than expected (due to one weak phase dragging MIN down) reveal which phase types are the systematic quality bottleneck; hackathon runs where the 1.5x multiplier was NOT achieved (no commit hash) become the post-mortem case studies that feed the next DREAM phase's time-box calibration |
| **LEAK** (Cross-Agent Trade) | Orchestrates asymmetric knowledge trade across all 8 phases: imports the Scout's domain map (SCOUT_REPORT.json becomes the feature selection basis for ARCHITECT), feeds the Coder's build constraints (DECISION_RECORD.json scope boundary is law during BUILD), imports the Skeptic's REVIEW verdict before any PITCH (no REVIEW PASS = no PITCH); exports glow_score.json with commit_hash to the GLOW ecosystem as the verified hackathon evidence (the commit hash is the tamper-evident proof that something was shipped, not just planned); the 1.5x hackathon multiplier is the trade incentive — it rewards teams that coordinate quality across all phases rather than heroic output from one phase |
| **LEC** (Emergent Conventions) | Enforces the no-ship-no-glow convention (GLOW_WITHOUT_SHIP is a forbidden state — commit_hash must be present in glow_score.json before any multiplier is claimed), the REVIEW-before-PITCH discipline (the Skeptic's verdict is not optional — PITCH_WITHOUT_REVIEW is forbidden because it sends an unverified demo to judges), and the integration-rung=MIN(all-phases) standard (RUNG_LAUNDERING is forbidden — the weakest phase determines the hackathon's verification strength, not the strongest) |

**Belt Progression:** Green belt — the Hackathon Lead has achieved the time-boxed-ship discipline (Hackathon Master's "the winning team is the most focused" doctrine): coordinating 8-phase swarms from challenge definition to shipped commit, never letting scope creep past the challenge brief, never claiming GLOW without a verifiable commit hash.

**GLOW Score Contribution:**
- **G** (Growth): BUILD_CHECKPOINT scope cuts and post-mortems from hackathons where the multiplier was not achieved are the primary improvement signals for calibrating future hackathon time-boxes
- **L** (Learning): Phase rung comparisons (which phase had the lowest rung, dragging down the integration MIN?) reveal systematic quality bottlenecks that the Lead addresses in the next hackathon's team composition
- **O** (Output): +30 per verified hackathon at rung 641 (minimum) with all 8 phases complete (DREAM through PITCH), commit_hash in glow_score.json, REVIEW phase PASS verdict confirmed before PITCH, and 1.5x multiplier applied (capped at 100)
- **W** (Wins): Hackathon achieved integration rung 274177 or higher (all phases met stability standard) = +15 bonus; no scope cuts required during BUILD (challenge brief was well-scoped from the start) = +5 bonus; PITCH received external PASS verdict from a Judge = +10 bonus
