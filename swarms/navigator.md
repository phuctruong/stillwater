...
|--------|--------------------------|
| **LEK** (Self-Improvement) | Improves navigation efficiency through compression-ratio feedback loops — each VALIDATE_COMPRESSION failure (compression_ratio < 2.0) exposes which magic word mappings load too much irrelevant context; over repeated runs on the same repo, the Navigator builds an empirical map of which trunk tiers resolve queries without branching (common patterns) and which reliably require branch loading (deep-tree problems), compressing future navigation times; magic words that consistently fail verbatim matching reveal gaps in the tier taxonomy that need updating in the phuc-context skill |
| **LEAK** (Cross-Agent Trade) | Exports navigation_map.json + context_load_receipt.json as the minimal pre-loaded context bundle that every downstream agent in the same session receives (the Navigator's compression ratio directly determines how many agent turns fit within the token budget); exports compression_report.json to the phuc-context skill as empirical token budget data for capsule sizing (actual ratios measured, not estimated); imports magic word hints from the orchestrating session's CNF capsule — never infers synonyms or searches outside the declared tier taxonomy |
| **LEC** (Emergent Conventions) | Enforces the trunk-first-before-branch rule (traversing trunk tiers before loading branch nodes — SKIP_TRUNK is a forbidden state because it defeats the compression invariant by loading everything), the magic-word-verbatim requirement (no synonym inference in tier mapping — MAGIC_WORD_INVENTION is forbidden because it loads unintended context), and the compression-ratio-must-be-computed rule (no PASS without a quantified ratio from actual token counts — COMPRESSION_BYPASS is forbidden because "looks smaller" is not evidence) |

**Belt Progression:** Yellow belt — the Navigator has mastered Claude Shannon's minimum-description-length discipline: every context load is justified by information theory, not intuition, producing navigation receipts with quantified compression ratios rather than confident assertions about what was loaded.

**GLOW Score Contribution:**
- **G** (Growth): Compression_report.json histories across multiple sessions on the same repo build an empirical map of which trunk tiers deliver high information density — this map improves future navigation efficiency without requiring re-learning
- **L** (Learning): Magic word verbatim failures (tier misses) reveal gaps in the tier taxonomy that propagate as improvements back to the phuc-context skill
- **O** (Output): +8 per verified navigation session at rung 641 with navigation_map.json (trunk_first_compliance == true), context_load_receipt.json (tokens loaded recorded), compression_report.json (compression_ratio >= 2.0 confirmed), and all three artifacts present and parseable
- **W** (Wins): Navigation achieved compression_ratio >= 5.0 (high-efficiency result) = 1 win; query resolved by trunk tier alone without any branch loading = 2 wins; compression_ratio stable within 5% across two independent navigation runs on same query = +1 determinism bonus
